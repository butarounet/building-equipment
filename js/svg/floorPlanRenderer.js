(function (root) {
  ...
  function fitDrawingToArea({ elementsBounds, targetArea, padding = 8, minScale = 0.002, maxScale = 0.02 } = {}) {
    ...
    const metricW = drawnW; const metricH = drawnH; const occupancy = (metricW * metricH) / Math.max(1, area.width * area.height); return { s, ox, oy, width: b.width, depth: b.height, drawnWidth: drawnW, drawnHeight: drawnH, metricWidth: metricW, metricHeight: metricH, occupancy, targetArea: area, x: (v) => ox + safe(v) * s, y: (v) => oy + safe(v) * s, l: (v) => safe(v) * s };
  }
  function txFor(plan) {
    const b = bounds(plan); const area = plan.floorId === '1' ? { x: 24, y: 18, width: 286, height: 228 } : { x: 24, y: 18, width: 314, height: 228 }; return fitDrawingToArea({ elementsBounds: { x: 0, y: 0, width: b.width, height: b.depth }, targetArea: area, padding: 6 });
  }
  ...
  function renderFurniture(items, t) {
    return (items || []).map((f, i) => p.drawRect({ id: `furniture-${escId(f.roomId || 'room')}-${escId(f.type || i)}-${i}`, x: t.x(f.x), y: t.y(f.y), width: Math.max(1.8, t.l(f.width)), height: Math.max(1.8, t.l(f.height)), fill: 'none', className: `line-thin furniture furniture-${escId(f.type || 'item')}`, strokeWidth: '0.10', dataRoomId: f.roomId }) + (['bath','basin','toilet'].includes(f.type) ? p.drawText({ id: `furniture-label-${i}`, x: t.x(f.x + f.width / 2), y: t.y(f.y + f.height / 2), text: f.name, className: 'text-note', fontSize: 2.5 }) : '')).join('');
  }
  ...
  function renderFloorPlan(floorPlan = {}, options = {}) {
    try {
      ...
      const rooms = autoRooms(plan, t); plan.furniture = plan.furniture || (furnitureEngine.createFurnitureLayout ? furnitureEngine.createFurnitureLayout(plan) : []); plan.dimensions = plan.dimensions || (dimEngine.createHotelDimensions ? dimEngine.createHotelDimensions(plan) : []); plan.paperUsageRatio = t.occupancy; const guestRoomCount = rooms.filter((r) => r.zone === 'guest').length; const metrics = quality.calculateFloorPlanQualityMetrics ? quality.calculateFloorPlanQualityMetrics({ floorPlan: plan }) : { paperUsageRatio: t.occupancy }; let svgDefs = patterns.createArchitecturalPatterns ? patterns.createArchitecturalPatterns() : ''; const arch = [];
      ...
      (plan.corridors || []).forEach((c, i) => arch.push(p.drawRect({ id: `corridor-${escId(c.id || c.corridorId || i + 1)}`, x: t.x(c.x), y: t.y(c.y), width: t.l(c.width), height: t.l(c.height), fill: 'none', className: `line-thin corridor corridor-${escId(c.type || 'main')}`, strokeWidth: 0.18 }) + p.drawText({ id: `corridor-label-${escId(c.id || c.corridorId || i + 1)}`, x: t.x(c.x + c.width / 2), y: t.y(c.y + c.height / 2), text: c.type === 'guest' ? '中廊下' : c.type === 'service' ? 'サービス廊下' : '主廊下', className: 'text-note', fontSize: 2.5 })));
      rooms.forEach((r, i) => { arch.push(p.drawRect({ id: `wall-room-${escId(r.roomId || i + 1)}`, x: t.x(r.x), y: t.y(r.y), width: t.l(r.width), height: t.l(r.height), fill: /PS|DS|EPS|浴室|便所|洗面/.test(r.name||'') ? 'url(#pattern-wet)' : 'none', className: r.zone === 'guest' ? 'line-medium wall-line guest-room' : r.zone === 'shaft' ? 'line-thin shaft-line' : 'line-medium wall-line', strokeWidth: r.zone === 'guest' ? 0.18 : r.zone === 'shaft' ? 0.22 : undefined, dataZone: r.zone, dataRoomId: r.roomId })); (r.windowPositions||[]).forEach((wpos, wi)=>arch.push(p.drawWindow({ id: `window-${escId(r.roomId)}-${wi}`, x: t.x(wpos.x), y: t.y(wpos.y), width: Math.max(5,t.l(wpos.width||1800)) }))); if(r.zone==='guest') arch.push(p.drawDoor({ id: `door-${escId(r.roomId)}`, x: t.x(r.x+r.width/2-450), y: t.y(r.y< t.depth/2 ? r.y+r.height : r.y), width: Math.max(3,t.l(900)), height: r.y<t.depth/2 ? 5 : -5 })); });
      ...
      return svg.replace('<svg ', `<svg data-paper-usage-ratio="${metrics.paperUsageRatio}" data-generated-guest-rooms="${guestRoomCount}" `);
