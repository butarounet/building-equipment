(function (root) {
  ...
  function renderSitePlan(sitePlan = {}, options = {}) {
    try {
      ...
      const s = Math.min(250 / safe(site.width, 90000), 175 / safe(site.height, 65000)); const ox = 55, oy = 55; const x = (v) => ox + (safe(v) - safe(site.x)) * s; const y = (v) => oy + (safe(v) - safe(site.y)) * s; const l = (v) => safe(v) * s;
      ...
      const text = [p.drawNorthArrow({ id: 'north-arrow-site', x: 365, y: 48 }), p.drawText({ id: 'entry-pedestrian', x: x(b.x + b.width / 2), y: y(b.y) - 3, text: '歩行者入口', className: 'text-note', fontSize: 2.5 }), p.drawText({ id: 'loading-dock-label', x: x(b.x + b.width) + 12, y: y(b.y + b.height * .55), text: '搬入口・サービス入口', textAnchor: 'start', className: 'text-note', fontSize: 2.5 }), p.drawText({ id: 'utility-electric', x: 310, y: 95, text: '受電引込位置', textAnchor: 'start', className: 'text-legend', fontSize: 2.5 }), p.drawText({ id: 'utility-gas-water', x: 310, y: 103, text: '都市ガス・上水引込位置', textAnchor: 'start', className: 'text-legend', fontSize: 2.5 }), p.drawText({ id: 'utility-sewer-rain', x: 310, y: 111, text: '下水接続・雨水排水位置', textAnchor: 'start', className: 'text-legend', fontSize: 2.5 }), p.drawText({ id: 'legend-site', x: 310, y: 190, text: '凡例：敷地境界・道路・建物・外構', textAnchor: 'start', className: 'text-legend', fontSize: 2.5 })].join('');
