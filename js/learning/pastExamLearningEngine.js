...
function analyzeWithPastExamLearning(input = {}, options = {}) { const result = learnPastExams(options); const styleScore = scoreExamStyle(input, result); return { ...result, score: Math.max(result.score, styleScore.score), warning: styleScore.score < 95, quality: { ...result.quality, candidate: styleScore, similarity: Math.max(result.score, styleScore.score), passed: Math.max(result.score, styleScore.score) >= 95 } }; }
class PastExamLearningEngine { constructor(options = {}) { this.options = options; this.knowledge = null; } learn(options = {}) { this.knowledge = learnPastExams({ ...this.options, ...options }); return this.knowledge; } analyze(input = {}, options = {}) { this.knowledge = this.knowledge || this.learn(options); return analyzeWithPastExamLearning(input, { ...this.options, ...options }); } persist(dir) { this.knowledge = this.knowledge || this.learn(); return persistLearningResult(this.knowledge, dir); } }
if (typeof module !== 'undefined') module.exports = { PastExamLearningEngine, learnPastExams, loadPastExams, persistLearningResult, analyzeWithPastExamLearning };
if (typeof window !== 'undefined') { window.PastExamLearningEngineClass = PastExamLearningEngine; window.PastExamLearningEngine = window.PastExamLearningEngine || new PastExamLearningEngine(); window.analyzeWithPastExamLearning = (...args) => window.PastExamLearningEngine.analyze(...args); }
