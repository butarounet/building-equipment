...
配置図、各階平面図、白図をDrawing GeneratorのJSONから編集可能SVGへ変換する建築図Rendererを追加した。今回の範囲は建築基本設計図風の白黒CAD表現に限定し、設備配管・ダクト・配線・模範解答は未描画とした。次工程では設備系統・答案レイヤーをLayer04/Layer06へ段階的に追加する。
