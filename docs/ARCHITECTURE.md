...
建築図Rendererは`architecturalDrawingRenderer`を入口にし、JSONの種類またはオプションに応じて`sitePlanRenderer`、`floorPlanRenderer`、`blankPlanRenderer`を呼び分ける。SVG基盤の7レイヤーを維持し、Layer01に建築要素、Layer02に通り芯、Layer03に寸法、Layer04に設備スペース名称のみ、Layer05に文字・凡例、Layer06は空、Layer07に図枠・タイトル枠を配置する。

ブラウザUIはBuilding Planner、Building Generator、Equipment Generator、Material Generator、Drawing Generator、Architectural Drawing Renderer、SVG表示の順で処理する。生成失敗、未生成データ、階未存在、室データ不足は画面メッセージとして扱い、JavaScriptエラーで画面全体を停止させない。
